...
btnTousElement.classList.add("filtreActive");
...
function genererFiltres () {
 ...
 for (let i = 0; i < btnFiltres.length; i++) {
    ...
    btnElement.classList.add("filtreNonActive");
    ...
    btnElement.addEventListener("click", function() {
        ...
        activerBtn();
        btnElement.classList.add("filtreActive");
    })
}}
...
function activerBtn() {
    const btnDeFilter = document.querySelectorAll(".filtres button");
    console.log(btnDeFilter);
    for (let i = 0; i < btnDeFilter.length; i++) {
        btnDeFilter[i].className = "filtreNonActive";
    }
    console.log(btnDeFilter);
}
...
function previewImage() {
    ...
    if(file.size > 4*1024*1024) {
        alert(`Erreur! La photo est trop grande"`);
    }else{
